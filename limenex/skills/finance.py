...
def make_charge(engine: PolicyEngine) -> Callable:
    ...
    async def charge(
        ...
        Policy dimensions:
            ...
            currency (str): Cannot be used as DeterministicPolicy.param — string
                values are not numeric. Use SemanticPolicy for currency-based
                rules (e.g. "Only allow USD transactions").
        ...
        Args:
            ...
            currency: ISO 4217 currency code (e.g. "USD", "GBP"). Forwarded to
                the executor as a routing hint; govern via SemanticPolicy if
                currency-based rules are required.
            executor: Developer-injected callable that performs the actual
                payment. Receives (amount=amount, currency=currency).
                agent_id is never forwarded. Sync and async callables are
                both supported.
    ...
    async def spend(
        ...
        Policy dimensions:
            amount_usd (float, projective): Use DeterministicPolicy(param="amount_usd")
                ...
                the policy.
            service (str): Cannot be used as DeterministicPolicy.param — string
                values are not numeric. Use SemanticPolicy for service-based
                rules (e.g. "Do not approve spend on unapproved services").
        ...
        Args:
            agent_id: The agent initiating this spend. Used by the engine
                to resolve and record policy state.
            service: The target service (e.g. "openai", "aws", "databroker").
                Forwarded to the executor as a routing hint; govern via
                SemanticPolicy if service-based rules are required.
            amount_usd: Spend amount in USD. Maps to
                DeterministicPolicy.param="amount_usd" for projective budget
                limit checks.
            executor: Developer-injected callable that performs the actual spend
                action. Receives (service=service, amount_usd=amount_usd).
                agent_id is never forwarded. Sync and async callables are
                both supported.
